    ...
    <div className={cn("space-y-4", className)}>
      <div className="flex items-center gap-x-4 gap-y-2 text-sm text-muted-foreground/70">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs uppercase tracking-wide font-medium">Question</span>
        </div>
      ...
      {options.map((opt) => {
        ...
        if (isAnswered) {
          if (revealCorrectness && isCorrect) {
            containerClasses = "p-4 rounded-lg border border-emerald-500/30 bg-emerald-500/5 cursor-default";
          } else if (isChosen) {
  ...
  return (
    <div className={cn("space-y-3", className)}>
      <div className="flex items-center gap-3 text-sm">
        ...
        {typeof maxMarks === "number" && maxMarks > 0 && (
          <span className="text-muted-foreground/50 text-xs">({maxMarks} marks)</span>
        )}
      ...
      {showReveal && onReveal && (
        <div className="flex items-center justify-end gap-2">
          ...
          <Button variant="ghost" size="sm" className="text-muted-foreground/70 hover:text-foreground" onClick={onReveal}>
