...
export function CompletionScreen({
  ...
  const { label: accuracyLabel, color: accuracyColor, bg: accuracyBg } = getAccuracyMeta(accuracyPercent);
      ...
      <CardContent className="px-5 py-6 space-y-6">
        ...
        <div className="flex flex-col items-center text-center space-y-3">
          ...
          <div className="text-sm text-muted-foreground">
            <span className="font-semibold text-foreground">{completedCount}/{totalCount}</span> correct
            <span className="mx-2">·</span>
            <span className="font-semibold text-foreground">{formattedElapsedTime}</span> total
          </div>
        </div>
        ...
        {(weakTopics.length > 0 || sessionCriteria.length > 0) && (
          <div className="space-y-3">
            <p className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground/60">What to review</p>

            {weakTopics.length > 0 && (
              <div className="space-y-1.5">
                {sessionTopics.filter(t => t.pct < 75).map(({ topic, pct }) => (
            ...
            {questionMode === "written" && sessionCriteria.length > 0 && (
              <div className="space-y-1.5">
                {sessionCriteria.map((c, i) => {
                  ...
                  return (
                    <div key={i} className={`flex items-start gap-2 rounded-lg border px-3 py-2 ${bg} border-transparent`}>
                      <AlertTriangle className={`w-3 h-3 shrink-0 mt-0.5 ${text}`} />
                      <div className="flex-1 min-w-0">
                        <p className="text-[11px] font-medium text-foreground leading-snug line-clamp-2"><MarkdownMath content={c.criterion} /></p>
                        <span className={`text-[10px] font-bold tabular-nums ${text}`}>{c.achieved}/{c.available} mk</span>
                      </div>
                    </div>
                  );
                })}
        ...
        <div className="border-t pt-3">
          ...
          >
            ...
            <ChevronDown className={`w-4 h-4 transition-transform duration-200 ${showDetails ? "" : "-rotate-90"}`} />
          </button>
