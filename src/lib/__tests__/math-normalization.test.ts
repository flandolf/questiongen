...
import { restoreMathPlaceholders,shieldMathForMarkdown } from '../math-normalization';
...
describe('math-normalization', () => {
  describe('shieldMathForMarkdown', () => {
    it('shields inline math', () => {
      ...
      expect(result.markdown).toMatch(/Solve QG_MATH_TOKEN_START_0_QG_MATH_TOKEN_END\./);
    ...
    it('shields display math', () => {
      ...
      const result = shieldMathForMarkdown(input);
      expect(result.markdown).toMatch(/Equation: QG_MATH_TOKEN_START_0_QG_MATH_TOKEN_END/);
  ...
  describe('restoreMathPlaceholders', () => {
    it('restores placeholders correctly', () => {
      ...
      const placeholders: Array<readonly [string, string]> = [['QG_MATH_TOKEN_START_0_QG_MATH_TOKEN_END', '$x + 1 = 2$']];
