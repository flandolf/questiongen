...
export function useGeneration() {
  ...
  const {
    ...
    mathMethodsSubtopics, specialistMathSubtopics,
    chemistrySubtopics, physicalEducationSubtopics,
    subtopicInstructions,
  } = useAppPreferences();

  const [streamText, setStreamText] = useState("");
  const [batchProgress, setBatchProgress] = useState<BatchTopicProgress[]>([]);
  const [lastSessionTelemetry, setLastSessionTelemetry] = useState<GenerationTelemetry | null>(null);

  const getSubtopicsForTopic = useCallback((topic: Topic): string[] => {
    switch (topic) {
  ...
  }, [mathMethodsSubtopics, specialistMathSubtopics, chemistrySubtopics, physicalEducationSubtopics]);
  ...
  const canGenerate = selectedTopics.length > 0 && apiKey.trim().length > 0 && model.trim().length > 0 && questionCount >= 1 && questionCount <= 20;

  const initBatchProgress = useCallback((topics: Topic[], counts: number[]): BatchTopicProgress[] => {
  ...
  return {
    ...
    lastSessionTelemetry, setLastSessionTelemetry,
    canGenerate,
