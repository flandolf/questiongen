...
function ExamRecordCard({
  ...
  const pct =
    record.totalMax > 0 ? (record.totalScore / record.totalMax) * 100 : 0;
            ...
            <div className="rounded-sm border bg-muted/20 p-3 space-y-1">
              ...
              <div className="text-[11px] text-muted-foreground">
                {record.questionCount > 0
                  ? `~${formatTime(Math.round(record.timeUsedSeconds / record.questionCount))}/q`
                  : ''}
              </div>
            </div>
            <div className="rounded-sm border bg-muted/20 p-3 space-y-1">
          ...
          <div>
            ...
            <div className="rounded-sm border border-border/40 divide-y divide-border/30 overflow-hidden">
              {record.questionResults.map((qr, i) => {
                const qPct =
                  qr.maxMarks > 0
                    ? (qr.achievedMarks / qr.maxMarks) * 100
                    : qr.correct
                      ? 100
                      : 0;
                return (
                  <div
                    key={i}
                    className={`flex items-center gap-3 px-3.5 py-2.5 text-sm ${qPct >= 100 ? 'bg-emerald-500/5' : qPct === 0 ? 'bg-rose-500/5' : ''}`}
                  >
                    <span className="shrink-0 w-5 text-[11px] font-mono text-muted-foreground">
                      {i + 1}
                    </span>
                    <div className="flex-1 min-w-0">
                      <div className="text-sm leading-snug line-clamp-1 prose prose-sm dark:prose-invert max-w-none">
                        <MarkdownMath
                          content={qr.promptMarkdown
                            .replace(/\n/g, ' ')
                            .slice(0, 120)}
                        />
                      </div>
...
export default function ExamHistoryView() {
  ...
  const filtered = useMemo(() => {
    let list = [...examHistory].sort((a, b) =>
      b.createdAt.localeCompare(a.createdAt)
    );
    if (modeFilter === 'written')
      list = list.filter((r) => r.questionMode === 'written');
    if (modeFilter === 'mc')
      list = list.filter((r) => r.questionMode !== 'written');
    return list;
  ...
  const stats = useMemo(() => {
    ...
    const avgScore =
      examHistory.reduce(
        (s, r) => s + (r.totalMax > 0 ? (r.totalScore / r.totalMax) * 100 : 0),
        0
      ) / totalExams;
    const best = examHistory.reduce((b, r) => {
      const pct = r.totalMax > 0 ? (r.totalScore / r.totalMax) * 100 : 0;
      return pct > (b.totalMax > 0 ? (b.totalScore / b.totalMax) * 100 : 0)
        ? r
        : b;
    }, examHistory[0]);
    const bestPct =
      best.totalMax > 0 ? (best.totalScore / best.totalMax) * 100 : 0;
  ...
  return (
    <PageContainer>
          ...
          {(['all', 'written', 'mc'] as const).map((m) => (
            ...
            >
              {m === 'all'
                ...
                : m === 'written'
                  ? `Written (${examHistory.filter((r) => r.questionMode === 'written').length})`
                  : `MC (${examHistory.filter((r) => r.questionMode !== 'written').length})`}
            </FilterButton>
          ))}
        </FilterGroup>
      </div>
      ...
      <ConfirmModal
        ...
        cancelText="Cancel"
        onConfirm={() => {
          if (deleteId) deleteExamRecord(deleteId);
          setDeleteId(null);
        }}
