            ...
            <div className="grid md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                {item.uploadedAnswerImage ? (
                  ...
                  <div className="whitespace-pre-wrap text-sm bg-muted/20 rounded-sm border border-border/30 px-3 py-2.5 min-h-16">
                    {item.uploadedAnswer ? (
...
export function HistoryView() {
  ...
  const filteredHistoryIdsKey = useMemo(
    () => filteredHistory.map((item) => `${item.kind}-${item.id}`).join('|'),
    [filteredHistory]
  );
