...
export function CleanupSection() {
  ...
  const mismatchGroups = useMemo(() => {
    ...
    for (const e of questionHistory)
      process(e.question.topic, e.question.subtopic);
        ...
        {fixCount !== null ? (
          <div className='flex items-start gap-6 p-8 rounded-2xl border border-emerald-500/20 bg-emerald-500/5'>
            ...
            <div className='space-y-1 min-w-0'>
              ...
              <p className='text-sm text-muted-foreground'>
                Updated {fixCount} {fixCount === 1 ? 'entry' : 'entries'} across
                your history.
              </p>
            </div>
            <Button
          ...
          <div className='flex items-start gap-6 p-8 rounded-2xl border border-emerald-500/20 bg-emerald-500/5'>
            ...
            <div className='space-y-1'>
              ...
              <p className='text-sm text-muted-foreground'>
                Every entry's subject matches its subtopic's canonical parent
                subject.
              </p>
            </div>
          </div>
          ...
          <div className='rounded-2xl border border-border bg-card'>
            <div className='px-8 pt-8 pb-4'>
              ...
              <p className='text-sm text-muted-foreground mt-2'>
                {totalMismatches === 1 ? 'entry has' : 'entries have'} a subject
                that doesn't match the canonical parent of its subtopic.
              </p>
            </div>
            ...
            <div className='flex items-center justify-between px-8 py-4 border-t border-border/40'>
              <p className='text-xs text-muted-foreground'>
                {totalMismatches} {totalMismatches === 1 ? 'entry' : 'entries'}{' '}
                across {mismatchGroups.length}{' '}
                {mismatchGroups.length === 1 ? 'subtopic' : 'subtopics'}
              </p>
              <Button
                onClick={handleFix}
