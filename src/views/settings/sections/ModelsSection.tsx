...
export function ModelsSection() {
  ...
  useEffect(() => {
    setLocalMarkingModel(markingModel);
  }, [markingModel]);
  useEffect(() => {
    setLocalUseSeparateMarkingModel(useSeparateMarkingModel);
  }, [useSeparateMarkingModel]);
  useEffect(() => {
    setLocalImageMarkingModel(imageMarkingModel);
  }, [imageMarkingModel]);
  useEffect(() => {
    setLocalUseSeparateImageMarkingModel(useSeparateImageMarkingModel);
  }, [useSeparateImageMarkingModel]);

  useEffect(() => {
    if (localModel && localModel !== model) setModel(localModel);
  ...
  useEffect(() => {
    if (localImageMarkingModel && localImageMarkingModel !== imageMarkingModel)
      setImageMarkingModel(localImageMarkingModel);
  }, [localImageMarkingModel, imageMarkingModel, setImageMarkingModel]);
  useEffect(() => {
    if (localUseSeparateImageMarkingModel !== useSeparateImageMarkingModel)
      setUseSeparateImageMarkingModel(localUseSeparateImageMarkingModel);
  }, [
    localUseSeparateImageMarkingModel,
    useSeparateImageMarkingModel,
    setUseSeparateImageMarkingModel,
  ...
  const fetchMarkingModelStats = useCallback(
  ...
  const fetchImageMarkingModelStats = useCallback(
    async (key: string, modelId: string) => {
      if (!key.trim() || !modelId.trim() || modelId === 'custom') return;
      setImageMarkingStatsLoading(true);
      setImageMarkingStatsError(null);
      setImageMarkingModelStats(null);
      try {
        const s = await invoke<ModelStats>('get_model_stats', {
          apiKey: key,
          modelId,
  ...
  useEffect(() => {
    if (apiKey && model) fetchModelStats(apiKey, model);
  }, [apiKey, model, fetchModelStats]);
  useEffect(() => {
    if (apiKey && useSeparateMarkingModel && markingModel)
      fetchMarkingModelStats(apiKey, markingModel);
  }, [apiKey, useSeparateMarkingModel, markingModel, fetchMarkingModelStats]);
  useEffect(() => {
    if (apiKey && useSeparateImageMarkingModel && imageMarkingModel)
      fetchImageMarkingModelStats(apiKey, imageMarkingModel);
  }, [
    apiKey,
    useSeparateImageMarkingModel,
    imageMarkingModel,
    fetchImageMarkingModelStats,
  ]);
  useEffect(() => {
  ...
  useEffect(() => {
    if (
      ...
      fetchMarkingModelStats(apiKey, localMarkingModel);
  }, [
    localMarkingModel,
    apiKey,
    localUseSeparateMarkingModel,
    fetchMarkingModelStats,
  ]);
  useEffect(() => {
    if (
      apiKey &&
      ...
      localImageMarkingModel !== 'custom'
    )
      fetchImageMarkingModelStats(apiKey, localImageMarkingModel);
  }, [
    localImageMarkingModel,
    apiKey,
    localUseSeparateImageMarkingModel,
  ...
  function openSearch(t: typeof searchTarget) {
    setSearchTarget(t);
    setSearchOpen(true);
  }
  ...
  const latestUpdate =
    statsUpdatedAt ?? markingStatsUpdatedAt ?? imageMarkingStatsUpdatedAt;
    ...
    <div className="space-y-6">
      ...
      <section className="space-y-3">
        <SectionHeader
          ...
          description="Used to generate questions and content."
        />
          ...
          <div className="rounded-lg border border-border bg-muted/50 p-3 space-y-1.5">
            ...
            <div className="flex flex-wrap gap-1.5">
              {[
                ...
                <span
                  key={name}
                  className="inline-flex items-center px-2 py-0.5 rounded bg-background text-xs font-mono border border-border"
                >
                  2025-{name}.pdf
                </span>
              ))}
      ...
      <section className="space-y-3">
        <SectionHeader
          title="Marking Model"
          description="Optionally use a separate model for grading student answers."
        />
        <ToggleRow
              ...
              <ImageModelSelectRow
                ...
                onSelect={(v) =>
                  v === 'custom'
                    ...
                    : (setShowCustomImage(false), setLocalImageMarkingModel(v))
                }
      ...
      <section>
        <div className="flex items-start justify-between mb-4">
          <div>
            ...
            <p className="mt-0.5 text-sm text-muted-foreground">
              Performance and pricing for the selected models.
            </p>
            {latestUpdate && (
          ...
          <div className="flex gap-1.5 shrink-0 ml-4 flex-wrap justify-end">
            {(
              [
                {
                  ...
                  m: localModel,
                  fn: () => fetchModelStats(apiKey, localModel),
                },
                ...(localUseSeparateMarkingModel
                  ? [
                      {
              ...
              <Button
                ...
                size="sm"
                className="gap-1.5"
                disabled={loading || !apiKey || !m || m === 'custom'}
                onClick={fn}
              >
                <RefreshCw
        ...
        {(statsError || markingStatsError || imageMarkingStatsError) && (
          <div className="mb-4 space-y-1.5">
            ...
            {imageMarkingStatsError && (
              <ErrorBanner message={imageMarkingStatsError} />
            )}
          </div>
        )}
        {!apiKey ? (
                ...
                ? [
                    {
                      ...
                      label: localMarkingModel || 'Marking',
                      loading: markingStatsLoading,
                    },
                  ]
                : []),
                ...
                ? [
                    {
                      ...
                      label: localImageMarkingModel || 'Image marking',
                      loading: imageMarkingStatsLoading,
                    },
                  ]
                : []),
