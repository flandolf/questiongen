...
export function StatsTable({ columns }: { columns: StatsColumn[] }) {
  const gridCols = (['', 'grid-cols-2', 'grid-cols-3', 'grid-cols-4'] as const)[
    columns.length
  ];
  return (
    <Card className='overflow-hidden'>
      <div
        className={cn(
          'grid text-xs font-medium text-muted-foreground bg-muted/50 px-4 py-2.5 border-b border-border',
          gridCols,
        )}
      >
        <span>Metric</span>
        {columns.map((c, i) => (
          <span key={i} className='truncate min-w-0' title={c.label}>
            {c.label}
          </span>
        ))}
      </div>
      <div className='divide-y divide-border'>
        {STAT_ROWS.map((row, ri) => (
          <div
            key={ri}
            className={cn(
              'grid items-center px-4 py-2.5 text-sm hover:bg-muted/30 transition-colors',
              gridCols,
            )}
          >
