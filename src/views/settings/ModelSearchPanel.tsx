...
import {
  Search, X, ArrowUpDown, ArrowUp, ArrowDown, RefreshCw, Image as ImageIcon,
} from "lucide-react";
...
export function ModelSearchPanel({ target, onClose, onSelect, apiKey }: ModelSearchPanelProps) {
          ...
          <p className="text-sm font-semibold flex items-center gap-2">
            ...
            {isImageTarget && (
              <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-full bg-sky-100 text-sky-700 dark:bg-sky-900/40 dark:text-sky-300 border border-sky-200 dark:border-sky-800 leading-none flex items-center gap-0.5">
                <ImageIcon className="h-2.5 w-2.5" />Vision only
              </span>
            )}
            {fromCache && !loading && (
